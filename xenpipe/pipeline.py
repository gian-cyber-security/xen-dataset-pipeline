...
def iter_xen_samples(dataset_id,*,target,split="train",revision=None,config=None,token=None,text_column=None,response_column=None,media_column=None,caption_column=None,max_samples=None,strict_license=True,allowed_licenses=None,deduplicate=True,image_size=None,cache_dir=None,resolve_media=False):
    """Stream normalized XEN samples; GEN1-V video stays lazy unless resolved."""
    ...
    for row in ds:
        if TARGETS[target]=="text":
            ...
            if not caption: continue
            ref=row[col]
            key=fingerprint_text(f"{dataset_id}|{base.revision}|{count}|{ref}")
            if deduplicate and key in seen: continue
            seen.add(key); p=asdict(base); p["target"]=target
